class LocalConditions
	...
	getWeather: ->
		...
		# lookup from xe.com
		fetch "http://api.worldweatheronline.com/free/v1/weather.ashx?q=Kunming&format=json&num_of_days=1&key=0d9295b0a177f7fb49506749d45f6445a3a66725"
		.then (response) ->
			response.json()
		.then (data) =>
			...
			if @el.weather
				...
				@el.weather.classList.add "o-icon--weather--" + icon
				@el.weather.querySelector("use").setAttribute "xlink:href", "#icon--" + icon
